...
生成されたテストケースが返ってくる。実際に出てきた結果はこんな感じだった。

```
### チケット: [MYPROJECT-123] ユーザー登録機能

#### テストケース1: 正常系 - 基本登録フロー
- **テスト観点**: 正常系
- **事前条件**: ユーザー登録画面が表示されている
- **テスト手順**:
  1. メールアドレスフィールドに「test@example.com」を入力
  2. パスワードフィールドに「SecurePass123!」を入力
  3. パスワード確認フィールドに「SecurePass123!」を入力
  4. 「登録」ボタンをクリック
- **期待結果**:
  - ユーザー登録が成功し、確認メールが送信される
  - ログイン画面にリダイレクトされる

#### テストケース2: 異常系 - メールアドレス形式エラー
- **テスト観点**: 異常系・バリデーション
- **事前条件**: ユーザー登録画面が表示されている
- **テスト手順**:
  1. メールアドレスフィールドに「invalid-email」を入力
...
#### テストケース3: 境界値 - パスワード最小文字数
- **テスト観点**: 境界値
- **事前条件**: パスワード最小文字数が8文字と定義されている
- **テスト手順**:
  1. パスワードフィールドに7文字のパスワードを入力
  2. パスワードフィールドに8文字のパスワードを入力
  3. それぞれで「登録」ボタンをクリック
- **期待結果**:
  - 7文字の場合: エラーメッセージが表示される
  - 8文字の場合: 正常に登録される

#### テストケース4: セキュリティ - パスワードの暗号化
- **テスト観点**: セキュリティ・データ保護
- **事前条件**: ユーザー登録が完了している
- **テスト手順**:
  1. データベースを確認
...
生成されたテストケースを確認し、必要に応じて調整してもらう。

- プロジェクト固有の観点が不足していれば追加
- 受け入れ条件に明記されていない暗黙の要件があれば補足
- テスト手順の具体性を高める

**ゼロからテストケースを作るのではなく、AIが生成した8割のベースを調整する作業に変わる。** これが一番大きな変化だった。QAメンバーからも「叩き台があるだけで全然違う」という声が出た。[Safieの事例](https://engineers.safie.link/entry/claude-code-test-automation-workflow-subagents)でも同様に「面倒なテストコードはAIに任せる」ワークフローが報告されている。
